    ...
    public static DeviceIdAndNameLists getDeviceIdAndNameLists(SensorType sensorType) {
        ...
        while (cursor.moveToNext()) {
            ...
            if (deviceTypeSet.contains(deviceType)) {
                ...
                if (deviceType == DeviceType.ROWING_POWER) {  // Ok, a Bike Power Sensor.  Here, we have to check the flags
    ...
    public static class DevicesDbHelper extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase db) {
            ...
            switch (DB_VERSION) {
                ...
                case 6:
                    ...
                    if (DEBUG) Log.d(TAG, "onCreated sql: " + CREATE_ROWING_POWER_SENSORS_TABLE_6);
                    break;
        ...
        private DeviceType getDeviceTypeFromANTDeviceTypeByte(byte antDeviceTypeByte) {
            switch (antDeviceTypeByte) {
                case (byte) 0x7A:
                    return DeviceType.ROWING_CADENCE;

                case (byte) 0x0B:
                    return DeviceType.ROWING_POWER;

                case (byte) 0x79:
                    return DeviceType.ROWING_SPEED_AND_CADENCE;

                case (byte) 0x7B:
                    return DeviceType.ROWING_SPEED;
